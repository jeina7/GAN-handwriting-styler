...
학습 초기단계인 30epoch까지는 모델이 전체적인 글자의 형상을 빠르게 잡을 수 있도록 L1 loss에 대해 더 큰 가중치를 두고 학습합니다. 그 후 150epoch까지 학습할 때는 constant loss에 더 큰 가중치를 둡니다. 이 Loss는 [DTN](https://arxiv.org/abs/1611.02200) 논문에서 소개된 손실함수로, 세부적인 부분에 대해 보다 효과적으로 학습됩니다.
...
150epoch의 사전학습이 끝난 모델로 손글씨를 학습합니다. 위 이미지는 151~550epoch 까지 학습하는 과정입니다. epoch 수가 많지만, 데이터가 210장으로 적어서 학습시간이 훨씬 짧습니다.
...
위에 쓰여있는 '내 손글씨를 따라 쓰는 인공지능' 13자는 모두 모델이 학습한 210자에 포함되지 않은 글자입니다. 즉, 모델은 사람이 직접 쓴 모든 글자를 다 학습하지 않고 일부만을 학습했지만, 나머지 글자들을 비슷한 스타일로 생성할 수 있습니다.
